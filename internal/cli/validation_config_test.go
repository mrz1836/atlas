...
func TestToValidationConfig_IncludesTemplateOverrides(t *testing.T) {
	cfg := &ValidationProviderConfig{
		FormatCmds: "gofmt -w .",
	...
	require.NotNil(t, valCfg.TemplateOverrides)
	assert.True(t, valCfg.TemplateOverrides["bugfix"].SkipTest)
	assert.False(t, valCfg.TemplateOverrides["bugfix"].SkipLint)
	assert.True(t, valCfg.TemplateOverrides["docs"].SkipTest)
	assert.True(t, valCfg.TemplateOverrides["docs"].SkipLint)
}

func TestInitializeDefaultTemplateOverrides(t *testing.T) {
	overrides := InitializeDefaultTemplateOverrides()

	// Should have all default template types
	expectedTypes := DefaultTemplateTypes()
	assert.Len(t, overrides, len(expectedTypes))

	for _, tmpl := range expectedTypes {
		override, exists := overrides[tmpl]
		assert.True(t, exists, "template %s should exist", tmpl)
		assert.False(t, override.SkipTest, "template %s should not skip tests by default", tmpl)
		assert.False(t, override.SkipLint, "template %s should not skip lint by default", tmpl)
	}
}

func TestDefaultTemplateTypes(t *testing.T) {
	types := DefaultTemplateTypes()

	assert.Contains(t, types, "bugfix")
	assert.Contains(t, types, "feature")
	assert.Contains(t, types, "refactor")
	assert.Contains(t, types, "docs")
}

func TestCollectValidationConfigNonInteractive_IncludesTemplateOverrides(t *testing.T) {
	result := &config.ToolDetectionResult{
		Tools: []config.Tool{},
	}

	cfg := CollectValidationConfigNonInteractive(result)

	// Should have template overrides initialized
	require.NotNil(t, cfg.TemplateOverrides)
	assert.Len(t, cfg.TemplateOverrides, len(DefaultTemplateTypes()))
}
