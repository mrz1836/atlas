...
func TestNewStartCmd(t *testing.T) {
	...
	fromBacklogFlag := cmd.Flags().Lookup("from-backlog")
	require.NotNil(t, fromBacklogFlag, "--from-backlog flag should be registered")
	assert.Empty(t, fromBacklogFlag.Shorthand, "from-backlog should not have shorthand")
	assert.Contains(t, fromBacklogFlag.Usage, "backlog discovery")
}
