...
func (r *Runner) executeTask(ctx context.Context, taskID string) {
	...
	defer func() {
		if rec := recover(); rec != nil {
			...
			r.markTaskFailed(ctx, taskID, fmt.Sprintf("panic: %v", rec))
			if r.events != nil {
				if pubErr := r.events.Publish(ctx, TaskEvent{
	...
	locked, lockErr := cache.WriteLock(ctx, r.redis, lockKey, r.workerID, 60)
	if lockErr != nil {
	...
	defer func() {
		if _, relErr := cache.ReleaseLock(ctx, r.redis, lockKey, r.workerID); relErr != nil {
